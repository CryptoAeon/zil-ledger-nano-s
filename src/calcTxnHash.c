...
static void fmtTxnElem(calcTxnHashContext_t *ctx) {
	...
	case TXN_ELEM_ZIL_OUTPUT:
		os_memmove(ctx->labelStr, "Zilliqa Output #", 16);
		bin2dec(ctx->labelStr+16, txn->sliceIndex);
		// An element can have multiple screens. For each zilliqa output, the
...
void handleCalcTxnHash(uint8_t p1, uint8_t p2, uint8_t *dataBuffer, uint16_t dataLength, volatile unsigned int *flags, volatile unsigned int *tx) {
	...
	if (p1 == P1_FIRST) {
		...
		//
		// NOTE: ctx->initialized is set to false when the Zilliqa app loads.
		if (ctx->initialized) {
		...
		// The official Zilliqa Nano S app only signs transactions on the forked
	...
	case TXN_STATE_FINISHED:
		if (ctx->sign) {
			...
			*flags |= IO_ASYNCH_REPLY;
			...
			// just prepare the APDU buffer and let zil_main call io_exchange?
			...
			// zil_main send the APDU with io_exchange. What happens at the
